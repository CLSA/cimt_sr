...
if(4 != count($argv))
{
  util::out('usage: process_merge input_ratings.csv input_sr.csv output.csv');
  exit;
}
...
$num_uid = count($opal_data);
util::out('number of opal data elements ' . $num_uid);
$changed_sites = array_unique($changed_sites);
util::out('changed sites: ' . util::flatten($changed_sites));
$site_names = array_unique($sites);
util::out('sites: ' . util::flatten($site_names));
...
foreach($set as $var)
{
  ...
  foreach($var_side as $key=>$side)
  {
    ...
    {
      $opal_id = $opal_data[$uid]['BARCODE']; // the true visit barcode
      $opal_site = $opal_data[$uid]['SITE'];
      $begintime = $opal_data[$uid]['DATETIME'];
      $date_valid = true;
      $site_valid = true;
      ...
      // acquisition datetime should be after minimum of the preceding Onyx interview stages:
...
$num_header = 1 + 2*(count($header_keys) + 1);
...
$default_rating['QUALITY']='';
$default_rating['PATIENTID']='';
$default_rating['ORIGINALID']='';
$default_rating['STUDYDATE']='';
$default_rating['STUDYTIME']='';
...
$num_rating_key = count($rating_keys);
$num_sr_key = count($sr_keys);

$index=1;
foreach($opal_data as $uid=>$data)
{
  ...
  foreach($set as $var)
  {
    foreach($var_side as $key=>$side)
    {
      ...
      if(array_key_exists($uid,${$var_name}))
      {
        ...
        $barcode_set[$key][] = $data['BARCODE']==$var_data['ORIGINALID'] ? 'match' : 'diff';

        if('rating' == $var)
        {
          if('NONE' != $var_data['CODE'])
          {
          ...
          if(0 == $var_data['QUALITY'] || '' == $var_data['QUALITY'])
          {  
            ${'num_' . $var . '_' . $key}--; 
          }
          else
          ...
          if(count($rating_data[$key])!=$num_rating_key)
          {
            var_dump($rating_data[$key]);
            die();
          }  
        }
        else
  ...
  foreach($var_side as $key=>$side)
  {
    ...
    if(count($rating_data[$key]) != $num_rating_key)
      util::out('ERROR: wrong number of rating keys '. count($rating_data[$key]) . ' < > ' . $num_rating_key); 
    
    foreach($rating_data[$key] as $id=>$value)    
      $str[] = $value;

    if(count($sr_data[$key]) != $num_sr_key)
      util::out('ERROR: wrong number of sr keys '. count($sr_data[$key]) . ' < > ' . $num_sr_key); 
  ...
  if($has_L || $has_R)
  {
    ...
    if($n_str != $num_header)
    {
      util::out('ERROR: wrong number of elements in output ' . $n_str . ' < > ' . $num_header );
    }
...
util::out($num_date_err . ' possible date errors among ' . $num_final_row . ' out of a maximim of ' . $num_uid);
util::out($num_datetime_err . ' possible datetime errors among ' . $num_final_row . ' out of a maximim of ' . $num_uid);
...
foreach($num_barcode_err_by_site as $site=>$num)
{
  ...
  util::out($num . ' / ' . $total . ' (' . round(100.0*$num/$total,2) . '%) sr barcode errors at '. $site );
}
